...
- No manual certificate installation is needed on the VPS. During the Docker build, the image downloads the official Russian Ministry of Digital Development root certificate and adds it to its own trusted certificate store. The download is accepted only when its SHA-256 fingerprint matches the value pinned in `scripts/install-mincifry-ca.sh`.
- Add the contents of [`deploy/Caddyfile.transfer.smjrfrb.ru`](deploy/Caddyfile.transfer.smjrfrb.ru) to the shared Caddyfile, then validate and reload Caddy. It accepts public HTTPS only at `transfer.smjrfrb.ru` and proxies the webhook to the container's loopback-only port `127.0.0.1:3600`, which does not conflict with the existing `3000` and `3500` services.
